...
-- | 'Control.Monad.Catch.catch' is transaction safe and implemented using
--   savepoints:
--   <https://www.postgresql.org/docs/current/static/sql-savepoint.html>.
--   Savepoints might have a noticable performance impact. The savepoints
--   are released when the transaction finishes or when the catch action and
--   handler finish execution, whichever is first. Savepoints are only used
--   if 'Control.Monad.Catch.catch' or a 'Control.Monad.Catch.catch' derived
--   function such as 'handle' is invoked.
...
runPGTransactionT :: MonadBaseControl IO m
                  ...
                  -> Postgres.Connection
                  -> m a
runPGTransactionT = runPGTransactionT' PT.DefaultIsolationLevel
